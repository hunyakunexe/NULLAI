...
def configure_torch_threads(torch, log_prefix='[gakushu]') -> int:
...
def optimize_gakushuing(moderu, optimizer, torch, enabled=True, bf16=True, log_prefix='[gakushu]'):
    ...
    try:
        ...
        moderu, optimizer = IPEX.optimize(moderu, **kwargs)
        print(f'{log_prefix} IPEX optimize(gakushuing)=ON; dtype={dtype}; weight_prepack=ON', flush=True)
        return moderu, optimizer, True
    except Exception as e:
        print(f'{log_prefix} IPEX optimize(gakushuing) unavailable ({e}); using PyTorch/oneDNN', flush=True)
        return moderu, optimizer, False


def optimize_inference(moderu, torch, enabled=True, log_prefix='[inference]'):
    if not enabled or IPEX is None:
        return moderu, False
    try:
        dtype = torch.bfloat16 if supports_bf16() else torch.float32
        moderu, _ = IPEX.optimize(moderu, dtype=dtype, inplace=True, weights_prepack=True)
        moderu.eval()
