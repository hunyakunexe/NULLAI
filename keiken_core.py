...
class ExperienceCore:
    ...
    def init(self):
        ...
        CREATE TABLE IF NOT EXISTS gakushuing_meta(key TEXT PRIMARY KEY,value TEXT);
        ''')
        self.db.commit()
    ...
    def search(self,semantic,ctx,limit=16,query=None):
        ...
        now=time.time(); out=[]
        qwords=set(re.findall(r'[\wぁ-んァ-ヶ一-龠]{2,}',query or ''))
        for r in rows:
            age=(now-r['ts'])/86400
            ...
            text=(r['input']+' '+r['response'])
            overlap=sum(1 for w in qwords if w in text)/max(1,len(qwords))
            score=(r['score']*.35+r['confidence']*.18+min(1,r['frequency']/10)*.12+sem*.15+match+overlap*.25)*decay
    ...
    def export_all_gakushuing_data(self):
        self.export()
