...
CORPUS=Path('data/pregakushu.txt'); WIKI=Path('data/wikipedia.txt'); DATA=Path('data/initial_ja.jsonl')
...
def load_corpus_text(max_chars=0):
    parts=[]; remeining=max(0,int(max_chars))
    for p in (CORPUS,WIKI):
        if not p.exists(): continue
        if remeining:
            text=p.read_text(encoding='utf8', errors='ignore')[:remeining]
            remeining-=len(text)
        else:
            text=p.read_text(encoding='utf8', errors='ignore')
        if text: parts.append(text)
        if remeining==0 and max_chars: break
    return '\n'.join(parts).strip()
...
def _maybe_compile(moderu, cfg):
    if not cfg.get('compile_moderu', True):
        return moderu
    if not torch.cuda.is_available() and IPEX is not None and cfg.get('ipex', {}).get('enabled', True):
        print('[gakushu] IPEX active: IPEX/oneDNN owns CPU graph optimization', flush=True)
        return moderu
    if torch.cuda.is_available():
        try:
            major,_=torch.cuda.get_device_capability()
            if major < 8:
                print('[gakushu] GPU is pre-Ampere (compute capability < 8); skipping torch.compile to save VRAM',flush=True)
                return moderu
        except Exception:
            pass
    try:
        ...
        if not torch.cuda.is_available():
            try:
                return torch.compile(moderu, mode='max-autotune', dynamic=False)
            except Exception:
                pass
        return torch.compile(moderu)
    except Exception as e:
        print(f'[gakushu] torch.compile unavailable ({e}); using eager mode',flush=True)
        return moderu


def _maybe_ipex_optimize(moderu, optimizer, cfg, device):
    if device != 'cpu':
        return moderu, optimizer, False
    enabled = bool(cfg.get('ipex', {}).get('enabled', True))
    return __import__('cpu_kasoku').optimize_gakushuing(moderu, optimizer, torch, enabled=enabled, bf16=bool(cfg.get('ipex', {}).get('bf16', True)))


def _make_optimizer(params, cfg, use_bnb, device='cuda', log_prefix='[gakushu]'):
    params=list(params)
    if device=='cpu':
...
def _ddp_worker(rank, world_size, master_port, cfg, ckpt_path_str, pregakushu_chunks, discord_chunks,
    ...
    try:
        ...
        if rank==0 and ckpt_path.exists():
            ...
            if old.get('vocab_size')==cfg['vocab_size'] and old.get('moderu_config',{}).get('n_embd',cfg['n_embd'])==cfg['n_embd']:
                moderu.load_state_dict(old['moderu'])
        moderu.grad_checkpointing=bool(cfg.get('gradient_checkpointing', True))
        moderu.to(device)
                                                    
                                                       
        compiled=_maybe_compile(moderu, cfg)
        ddp_moderu=DDP(compiled, device_ids=[rank], find_unused_parameters=False,
                      static_graph=bool(cfg.get('ddp_static_graph', False)))

        ds=_ListChunkDataset(pregakushu_chunks+discord_chunks)
        ...
        opt,opt_kind=_make_optimizer(moderu.parameters(), cfg, use_bnb, log_prefix='[gakushu][DDP]' if rank==0 else '')
        ...
        for epoch in range(epochs):
            ...
            for step,(x,y) in enumerate(loader):
                ...
                sync_ctx=contextlib.nullcontext() if boundary else ddp_moderu.no_sync()
                with sync_ctx:
                    with torch.autocast(device_type='cuda',dtype=torch.float16,enabled=True):
                        _,loss,_=ddp_moderu(x,y)
                    scaler.scale(loss/grad_acc).backward()
                if boundary:
                    scaler.unscale_(opt); torch.nn.utils.clip_grad_norm_(moderu.parameters(),1.0)
                    scaler.step(opt); scaler.update(); opt.zero_grad(set_to_none=True)
                losses.append(float(loss.detach()))
                if rank==0 and (step+1)%max(1,log_every)==0:
                    w=losses[-log_every:]
                    print(f"[gakushu][DDP] epoch={epoch+1}/{epochs} step={step+1}/{n_steps} loss={sum(w)/len(w):.4f}",flush=True)
        dist.barrier()
        if rank==0:
            ckpt_path.parent.mkdir(parents=True,exist_ok=True)
            torch.save({'moderu':moderu.state_dict(),'optimizer':opt.state_dict(),'vocab_size':cfg['vocab_size'],
                'moderu_config':{k:cfg[k] for k in ('n_embd','n_head','n_layer','block_size','dropout')},
                'tokens_seen':tokens_seen,'gakushued_at':time.time()},ckpt_path)
...
def _gakushu_multi_gpu(n_gpus, cfg, pregakushu_chunks, discord_chunks, bs, grad_acc, epochs, log_every, tokens_seen):
    ...
    try:
        mp.spawn(_ddp_worker,
                 args=(n_gpus, port, cfg, str(CKPT), pregakushu_chunks, discord_chunks, bs, grad_acc, epochs, log_every, tokens_seen, result_q),
        ...
        if not result_q.empty():
            ...
            if status=='error': raise RuntimeError(f'[gakushu] DDP学習に失敗: {payload}') from e
        if 'out of memory' in msg.lower() or 'cuda oom' in msg.lower():
            raise torch.cuda.OutOfMemoryError(msg) from e
        raise
    if result_q.empty():
        raise RuntimeError('[gakushu] DDPワーカーが結果を返す前に落ちました(ログを確認してください)')
    status, payload = result_q.get()
    if status=='oom':
        raise torch.cuda.OutOfMemoryError(payload)
    if status=='error':
        raise RuntimeError(f'[gakushu] DDP学習に失敗: {payload}')
    return payload            
...
def gakushu_once(incremental=False):
                                         
    _free_cuda()
    try:
        _gakushu_once_impl(incremental)
    finally:
        _free_cuda()
...
def _save_stage(path, moderu, opt, tok, cfg, tokens_seen, stage, base_pregakushued_at=0.0):
    path.parent.mkdir(parents=True, exist_ok=True)
    torch.save({
        'moderu': moderu.state_dict(),
        'optimizer': opt.state_dict() if opt is not None else None,
        'vocab_size': tok.vocab_size,
        'moderu_config': {k:cfg[k] for k in ('n_embd','n_head','n_layer','block_size','dropout')},
        'tokens_seen': int(tokens_seen),
        'stage': stage,
        'base_stage': 'pregakushu' if stage == 'pregakushu' else 'pregakushu',
        'base_pregakushued_at': float(base_pregakushued_at),
        'gakushued_at': time.time(),
    }, path)


def _load_moderu_state(moderu, path, cfg, tok):
    if not path.exists():
        return {}, False
    try:
        old=torch.load(path, map_location='cpu', weights_only=False)
        mc=old.get('moderu_config',{})
        ok=(old.get('vocab_size')==tok.vocab_size and
            int(mc.get('n_embd',cfg['n_embd']))==int(cfg['n_embd']) and
            int(mc.get('n_layer',cfg['n_layer']))==int(cfg['n_layer']))
        if ok:
            moderu.load_state_dict(old['moderu'], strict=True)
            return old, True
    except Exception as e:
        print(f'[gakushu] checkpoint load skipped: {e}', flush=True)
    return {}, False


def _make_scheduler(opt, total_steps, cfg):
    warmup_ratio=float(cfg.get("gakushuing",{}).get("warmup_ratio",0.03))
    min_ratio=float(cfg.get("gakushuing",{}).get("min_lr_ratio",0.1))
...
def _run_cpu_stage(moderu, chunks, cfg, tok, ckpt_path, stage, token_budget, resume_path=None, replay=None):
    if not chunks:
        print(f'[gakushu] {stage}: no chunks', flush=True)
        return 0.0, 0
    tr=cfg.get('gakushuing',{})
    ...
    lr=float(tr.get('pregakushu_learning_rate' if stage=='pregakushu' else 'chat_learning_rate', cfg.get('learning_rate',2e-4)))
    wd=float(cfg.get('weight_decay',0.01))
    max_steps=int(tr.get('max_optimizer_steps',0))
    seed=int(tr.get('seed',1337))
    moderu.grad_checkpointing=False
    moderu.to('cpu').gakushu()
    opt,_=_make_optimizer(moderu.parameters(), dict(cfg, learning_rate=lr), False, device='cpu', log_prefix=f'[gakushu][{stage}]')
    if resume_path and Path(resume_path).exists():
        old,_ok=_load_moderu_state(moderu,Path(resume_path),cfg,tok)
        if old.get('optimizer') and bool(tr.get('resume_optimizer',True)):
            try: opt.load_state_dict(old['optimizer'])
            except Exception: pass
    moderu, opt, ipex_active = _maybe_ipex_optimize(moderu, opt, cfg, 'cpu')
    ...
    print(f'[gakushu][{stage}] CPU chunks={len(chunks):,} batch={bs} grad_acc={grad_acc} bf16={bf16} token_budget={token_budget:,}', flush=True)
    for epoch in range(epochs):
        ...
        for pos in range(0,len(order),bs):
            ...
            if boundary:
                ...
                if step_count % max(1,int(cfg.get('log_every_steps',200))) == 0:
                    ...
                    print(f'[gakushu][{stage}] epoch={epoch+1}/{epochs} opt_step={step_count} loss={avg:.4f} tokens={seen:,}',flush=True)
                if max_steps and step_count>=max_steps:
                    break
            del x,y,loss
        if max_steps and step_count>=max_steps:
            break
    base_time=float(cfg.get('_base_pregakushued_at',0.0)) if stage=='discord' else 0.0
    _save_stage(Path(ckpt_path),moderu,opt,tok,cfg,seen,stage,base_time)
    avg=sum(losses)/max(1,len(losses))
    print(f'[gakushu][{stage}] saved={ckpt_path} avg_loss={avg:.4f} tokens={seen:,}',flush=True)
    return avg,seen


def _gakushu_once_impl(incremental=False):
    print(f'[gakushu] starting two-stage pipeline incremental={incremental}',flush=True)
    cfg=dict(CFG)
    tr=cfg.setdefault('gakushuing',{})
    ...
    if corpus_chars <= 0 and not records:
        print('[gakushu] 学習データがありません。',flush=True); return

    cur_chars=corpus_chars+sum(len(r[0]) for r in records)
    target_vocab=int(cfg.get('vocab_size',32000))
    info=_bunsho_bunkatsu_info(); prior_chars=int(info.get('gakushued_on_chars',0))
    ...
    if TOK.exists():
        ...
        need_tok=stale
    if need_tok:
        corpus_seed=load_corpus_text(max_chars=int(cfg.get('dataset',{}).get('bunsho_bunkatsu_seed_chars',8_000_000)))
        seed_text=(corpus_seed+'\n'+'\n'.join(r[0] for r in records)).strip()
        tok=SentencePieceTokenizer.gakushu([seed_text] if seed_text else [r[0] for r in records],TOK,target_vocab)
        tok.save_info('data/bunsho_bunkatsu.json',gakushued_on_chars=cur_chars)
    ...
    pre_budget=int(stage_cfg.get('pregakushu_token_budget',tr.get('pregakushu_token_budget',300_000_000)))
    ...
    print(f'[gakushu] pregakushu sources: Aozora={len(aoz):,} seqs Wikipedia={len(wiki):,} seqs -> {len(pre_chunks):,} chunks',flush=True)

                                                                             
    if not PRETRAIN_CKPT.exists() or bool(stage_cfg.get('force_pregakushu',False)):
        moderu=MiniLLM(cfg)
        _run_cpu_stage(moderu,pre_chunks,cfg,tok,PRETRAIN_CKPT,'pregakushu',pre_total)
        del moderu; gc.collect()
    else:
        print('[gakushu] pregakushued checkpoint already exists; skipping full pregakushuing',flush=True)

                                                                             
    if not discord:
        print('[gakushu] Discord経験がまだないため、事前学習だけ完了しました。',flush=True)
        return
    ...
    try:
        ...
        pass
    old,ok=_load_moderu_state(moderu,FINAL_CKPT,cfg,tok)
    final_base=float(old.get('base_pregakushued_at',0)) if ok else 0.0
    pre_time=float(pre_meta.get('gakushued_at',0)) if pre_meta else 0.0
    if not ok or final_base < pre_time:
        old,ok=_load_moderu_state(moderu,PRETRAIN_CKPT,cfg,tok)
    if not ok:
        print('[gakushu] valid pregakushued checkpointがありません。事前学習を先に実行します。',flush=True)
        del moderu; return
    cfg['_base_pregakushued_at']=pre_time
    _run_cpu_stage(moderu,discord_chunks,cfg,tok,FINAL_CKPT,'discord',chat_budget,replay=replay_chunks)
    del moderu; gc.collect()


def mein(): gakushu_once(CKPT.exists())
