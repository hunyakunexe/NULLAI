...
moderu=MiniLLM(cfg); ck=torch.load("data/moderu.pt",map_location="cpu",weights_only=False); moderu.load_state_dict(ck["moderu"]); moderu.eval()
if IS_INTEL_CPU and IPEX is not None and cfg.get("ipex", {}).get("enabled", True):
    try:
        moderu=IPEX.optimize(moderu, dtype=torch.bfloat16, inplace=True, weights_prepack=True)
        moderu.eval()
        print("[inference] IPEX optimize=ON", flush=True)
    except Exception as e:
        print(f"[inference] IPEX optimize unavailable ({e})", flush=True)
if cfg.get("inference",{}).get("quantized",False) and quantize_dynamic and not (IS_INTEL_CPU and IPEX is not None): moderu=quantize_dynamic(moderu,{torch.nn.Linear},dtype=torch.qint8); moderu.eval()
def seisei(prompt):
    ids=torch.tensor([tok.encode(prompt)],dtype=torch.long); out=moderu.seisei(ids,max_new_tokens=int(cfg.get("inference",{}).get("max_new_tokens",120)),temperature=.85,top_k=30)
                                                        
    return tok.decode(out[0].tolist()[len(ids[0]):])
if __name__=="__mein__": print(seisei(input(">>> ")))
